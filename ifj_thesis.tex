...
%if you want special chapter formatting, you'll probably want to avoid using it altogether
  ...
  \label{sub:Algortithm Sketch}
    ...
    So, given a set of sites and a desire to compute the Delaunay triangulation of that set, we begin. Though \say{divide and conquer} might lead you to believe that this approach consists of two stages, I prefer to think of it as three. First, we have the \say{split,} or \say{divide,} stage, in which we split the large problem of computing the Delaunay of $P$ into smaller subproblems. We continue to do this until we are operating on a collection of subproblems of a certain size. At this point, we proceed to the second stage in which we triangulate the trivial subproblems. Finally, we proceed to the \say{merge} phase, which is the process of strategically combining the results of the subproblems into the final triangulation on $n$ sites. 
  ...
  \label{subsub:Divide}
    ...
    To choose the x-values which will govern our partitions, we sort the sites by x-coordinate and choose the median. \Cref{fig:del_partition0} depicts the vertical lines which partition $P$ into subproblems where the number of sites is either two or three.  Note that in this example, some of the sites have very similar x-coordinates. In cases where we can't assume x-coordinates are unique, we may break the \say{tie}, so to speak, by comparing the y-coordinates of the sites. \Cref{fig:del_partition1} illustrates how subproblems of size two or three are triangulated. \par 
    \begin{figure}[!htb]
    \centering
  ...
  \State $L, R,  \gets \Call{PartitionSites}{$P$}$\Comment{Partition $P$ by median x-coordinate}
  \State $Del(L) \gets $\Call{DelDivConq}{$L$}  \Comment{Recursive call}
  \Statex[1] $Del(R) \gets $\Call{DelDivConq}{$R$}
  \State \Call{DelMerge}{$Del(L)$, $Del(R)$}  \Comment{Merge sub-diagrams}
  \EndProcedure
  \end{algorithmic} 
  \end{algorithm}
  ...
  \Procedure{DelMerge}{$Del(L)$, $Del(R)$}
  \EndProcedure
  \end{algorithmic} 
  \end{algorithm}
