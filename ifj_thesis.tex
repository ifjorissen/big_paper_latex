...
% \onehalfspacing
  ...
  \label{sub:a_graph_rep}
    ...
    \subsection{The Delaunay Graph} % (fold)
...
% subsection the_actors_in_our_play (end)
  ...
  \label{sub:gs_sketch}
    ...
    \Else \Comment{Recursive Case}
      \State $x\gets$\Call{Median-Lex}{$\{x\mid(x,y)\in\;P\}$}\label{rc}
    ...
    We assume three things of $P$: that the $x$-coordinate of each site in $P$ is unique; that the sites are in \emph{general position},\footnote{As discussed in \cref{sub:general_position}.} meaning that there will be no degenerate cases for us to handle; and that not all sites are collinear, a special case we covered in \cref{sub:special_case}. Given $P$, \nameref{divconq_del} computes $\mbox{Del}(P)$ by splitting the problem into subproblems and then merging the results pairwise. Lines \ref{bc_1} and \ref{bc_2} handle the base cases of the algorithm, which we will discuss in-depth in \cref{sub:handling_the_base_cases}. \par

    The final chunk of pseudocode, lines \ref{rc} to \ref{fin}, handle the recursive case. When there are more than three sites in $P$, we need to break the problem down further. We call \textsc{Median-Lex}, which takes set of the $x$-coordinates from the sites in $P$ and returns the median value. In line \ref{split}, we pass this value--- along with $P$--- to another helper function, \textsc{Split}. \textsc{Split} returns two sets of sites, $L$ and $R$ (where $L = {p \mid p.x \leq x}$). Then, we make the calls to \nameref{divconq_del} on both $L$ and $R$. Finally, in line \ref{merge}, we merge the results with a call to \nameref{del_merge} and return the result. In \cref{sub:merging}, we dive into the \par
  ...
  \label{sub:merging}
    ...
    % \nameref{del_merge} illustrates the pseudocode for this process. It will function as a reference and a generalization of the steps we will walk through to merge $D_{L}$ and $D_{R}$. 
    ...
    %   \State $basel\gets$ \Call{Lower-Common-Tangent}{$ldi$, $rdi$}\label{lct}
    %   \If {$ldi.org$ is $ldo.org$}
    %     \State $ldo\gets basel.twin$
    %   \EndIf
    %   \If {$rdi.org$ is $rdo.org$}
    %     \State $rdo\gets basel$
    %   \EndIf
    %   \Loop \label{rb}
    %     \State $\ell\gets basel.twin.onext$
    ...
    %       \State $basel\gets$ \Call{Connect}{$r$, $basel.twin$} \label{ce_add1}
    %     \Else
    %       \State $basel\gets$ \Call{Connect}{$basel.twin$, $\ell.twin$} \label{ce_add2}
    %     \EndIf
    ...
    \subsubsection{The First Cross-Edge} % (fold)
    \label{ssub:the_first_cross_edge}
      ...
      \begin{algorithm}[!htb]
        ...
        \Loop
          ...
          \Else
            \State $basel\gets$\Call{Connect}{rdi.twin, ldi}
    ...
    \subsubsection{The Rising Bubble} % (fold)
    \label{ssub:the_rising_bubble}
      ...
      \begin{figure}[!htb]
        ...
        \begin{subtable}{\textwidth}
          ...
          \caption{Evaluating whether or not $\ell$ will be in $D_{P}$. The dashed arc is a portion of the \emph{rising bubble} which we say has \say{witnessed} that $v$ in not in the circle defined by $\ell.twin.org$, $basel.twin.org$, and $basel.org$.}
          \label{fig:del_knit1}
        \end{subtable}
      \end{figure}


      % What we need to do is consider whether or not any of edges from \emph{$\ell.org$}-- including $\ell$-- intersect with the cross edge we'd like the insert. To do this we use the \textsc{InCircle} test, the second geometric predicate that Guibas and Stolfi use. 
      ...
      \begin{figure}[!htb]
        ...
        \begin{subtable}{\textwidth}
          ...
          \caption{Evaluating whether or not $r$ will be in $D_{P}$. The dashed arc is a portion of the ``rising bubble'' which we say has \emph{witnessed} that $u$ in not in the circle defined by $r.twin.org$, $basel.twin.org$, and $basel.org$.}
          \label{fig:del_knit3}
        \end{subtable}
      ...
      %   \centering
        \begin{subtable}{\textwidth}
          ...
          \caption{Since $\ell$ and $r$ are both valid we perform \textsc{InCircle}($\ell.twin.org$, $\ell.org$, $r.org$, $r.twin.org$). Note that $r.twin.org$ is not in the circle formed by $\ell.twin.org$, $\ell.org$, $r.org$, so \textsc{InCircle} returns false.}
          \label{fig:del_knit4}
        \end{subtable}
        %
      % \end{figure}
      ...
      %   \centering
        \begin{subtable}{\textwidth}
          ...
          \caption{Due to the result of the \textsc{InCircle} test, we update \emph{basel}'s destination to $\ell.twin.org$. We also update $\ell$ to $basel.twin.onext()$.}
          \label{fig:del_knit5}
        \end{subtable}
      \end{figure}
      ...
      \begin{figure}[!htb]
        ...
        \begin{subtable}{\textwidth}
          ...
          \caption{We continue this process noting that $\ell$ is a valid edge. Because this is the case, we check to see if $v$, that is, $\ell.onext().twin.org$, is contained in the circle represented by the dashed lines. It is.}
          \label{fig:del_knit6}
        \end{subtable}
      ...
      \begin{figure}[!htb]
        ...
        \begin{subtable}{\textwidth}
          ...
          \caption{We consider the right side again, noting that $r$ is still valid. We may note that $u$ has not been reassigned. However, since $basel$ has changed, we need to check that $u$ is not in the circle formed by $r.twin.org$, $basel.twin.org$, and $basel.org$. Here, it is not.}
          \label{fig:del_knit9}
        \end{subtable}
      ...
      %   \centering
        \begin{subtable}{\textwidth}
          ...
          \caption{Again, since $\ell$ and $r$ are both valid we perform \textsc{InCircle}($\ell.twin.org$, $\ell.org$, $r.org$, $r.twin.org$). Note that $r.twin.org$ is in the circle formed by $\ell.twin.org$, $\ell.org$, $r.org$, so \textsc{InCircle} returns true.}
      ...
      %   \centering
        \begin{subtable}{\textwidth}
          ...
          \caption{Because the \textsc{InCircle} test returned true, we update $basel$ so that its source is now $r.twin.org$. We also add the old $basel$ edge to $D_{P}$. Finally, we update $r$.}
    ...
    \label{ssub:algorithm_pseudocode}
      ...
      \begin{algorithm}[!htb]
        ...
        \Loop \label{rb} \Comment{Rising bubble portion}
          \State $\ell\gets b.twin.onext$
          \If{\Call{Valid}{$\ell$}} \label{valid_l}
            \While{\Call{InCircle}{$b.dest$, $b.org$, $\ell.dest$, $\ell.onext.dest$}}\label{rm_l}
              \State $\ell' \gets \ell.onext$
              \State \Call{DeleteEdge}{$\ell$}
              \State $\ell \gets \ell'$
            \EndWhile
          \EndIf
          \State $r \gets b.oprev$
          \If{\Call{Valid}{$\ell$}}
            \While{\Call{InCircle}{$b.dest$, $b.org$, $r.dest$, $r.oprev.dest$}}\label{rm_r}
              \State $r' \gets r.oprev$
              \State \Call{DeleteEdge}{$r$}
              \State $r \gets r'$
            \EndWhile
          \EndIf
          \If{not \Call{Valid}{$\ell$} and not \Call{Valid}{$r$}} \Comment{$b$ is UCT}\label{ce}
            \State exit loop
          \EndIf
          \If{not \Call{Valid}{$\ell$} or \par
            \hskip\algorithmicindent\hskip\algorithmicindent(\Call{Valid}{$r$} and \Call{InCircle}{$\ell.dest$, $\ell.org$, $r.org$, $r.dest$})}\label{ce_1}
            \State $b\gets$ \Call{Connect}{$r$, $b.twin$} \label{ce_add1}
          \Else
            \State $b\gets$ \Call{Connect}{$b.twin$, $\ell.twin$} \label{ce_add2}
          \EndIf
        \EndLoop \label{rb_end}
        \If {$ldi.org$ is $ldo.org$}
      ...
      % \begin{figure}[!htb]
        ...
        %   \centering
        %   \input{figs/ch1/guibas/knit0.tex}
        %   \caption{First edge, $basel$, of $D_{P}$ and $\ell$, candidate edge in $D_{L}$. Note that $\ell$ is considered to be valid because \textsc{CCW}($\ell.twin.org$, $basel.twin.org$, $basel.org$) is true. That is to say, $\ell.twin.org$, $basel.twin.org$, $basel.org$ is a counterclockwise ordering.}
        %   \label{fig:del_knit0}
        % \end{subtable}
      ...
      %   \centering
        ...
        %   \centering
        %   \input{figs/ch1/guibas/knit1.tex}
        %   \caption{Evaluating whether or not $\ell$ will be in $D_{P}$. The dashed arc is a portion of the ``rising bubble'' which we say has \emph{witnessed} that $v$ in not in the circle defined by $\ell.twin.org$, $basel.twin.org$, and $basel.org$.}
        %   \label{fig:del_knit1}
        % \end{subtable}
      % \end{figure}
      % \begin{figure}[!htb]
...
% subsection handing_the_base_cases (end)
  ...
  % \label{sub:divide_and_conquer_algorithm_pseudocode}
    ...
    %   \State $D_{L} \gets$ \Call{GS-Delaunay}{$L$}
    %   \Statex[2] $D_{R} \gets$ \Call{GS-Delaunay}{$R$}
    %   \State $D\gets$ \Call{Merge}{$D_{L}$, $D_{R}$}  \Comment{Merge sub-diagrams}
